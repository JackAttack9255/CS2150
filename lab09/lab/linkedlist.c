...
int main(){
  ...
  struct list_item* b = malloc(sizeof(struct list_item));
  if(b == NULL) {
    printf("memory allocation failed");
    return 1;
  }
  ...
  struct list_item* x = f;
  struct list_item* itr = f->next;
